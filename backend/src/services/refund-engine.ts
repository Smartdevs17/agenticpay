import { randomUUID } from 'node:crypto';
import { ok, err, type Result } from '../lib/result.js';
import { AppError } from '../middleware/errorHandler.js';
import { ImmutableAuditLogger } from '../audit/immutable-logger.js';

// ── Types ────────────────────────────────────────────────────────────────────

export type RefundStatus =
  | 'pending'
  | 'approved'
  | 'rejected'
  | 'processing'
  | 'completed'
  | 'failed'
  | 'cancelled';

export type RefundDecision = 'approved' | 'manual_review' | 'rejected';

export type PaymentMethod = 'card' | 'crypto' | 'bank_transfer';

export type CustomerTier = 'basic' | 'premium' | 'enterprise';

export interface PolicyRule {
  id: string;
  field: string;
  operator: 'eq' | 'neq' | 'gt' | 'gte' | 'lt' | 'lte' | 'in' | 'not_in' | 'contains';
  value: unknown;
  outcome: 'approve' | 'reject' | 'manual_review';
  priority: number;
}

export interface RefundPolicy {
  id: string;
  workspaceId: string;
  name: string;
  fullRefundWindowDays: number;
  autoApprovalThreshold: number;
  alwaysRefundUnderAmount: number;
  maxPartialRefundPct: number;
  requireReason: boolean;
  firstApprovalThreshold: number;
  secondApprovalThreshold: number;
  rules: PolicyRule[];
  isActive: boolean;
  createdAt: string;
  updatedAt: string;
}

export interface RefundEvaluationInput {
  workspaceId: string;
  paymentId: string;
  paymentType: PaymentMethod;
  amountPaid: number;
  requestedAmount: number;
  currency: string;
  daysSincePayment: number;
  reason?: string;
  hasChargeback: boolean;
  hasDispute: boolean;
  customerTier?: CustomerTier;
  customerTotalSpent?: number;
  previousRefundCount?: number;
  previousRefundAmount?: number;
  lineItems?: Array<{ label: string; amount: number; quantity: number; reason?: string }>;
}

export interface RefundApproval {
  level: 'first' | 'second' | 'third';
  approverId: string;
  status: 'pending' | 'approved' | 'rejected';
  comment?: string;
  approvedAt?: string;
}

export interface RefundHistoryEntry {
  timestamp: string;
  action: string;
  actor: string;
  fromStatus: RefundStatus;
  toStatus: RefundStatus;
  comment?: string;
}

export interface RefundRecord {
  id: string;
  workspaceId: string;
  paymentId: string;
  paymentType: PaymentMethod;
  stripeRefundId?: string;
  amount: number;
  currency: string;
  reason?: string;
  status: RefundStatus;
  decision?: RefundDecision;
  decisionReasons: string[];
  requestedBy?: string;
  approvedBy?: string;
  processedAt?: string;
  completedAt?: string;
  cancelledAt?: string;
  failureReason?: string;
  autoProcessed: boolean;
  lineItems: Array<{ label: string; amount: number; quantity: number; reason?: string }>;
  approvals: RefundApproval[];
  history: RefundHistoryEntry[];
  metadata?: Record<string, unknown>;
  createdAt: string;
  updatedAt: string;
}

export interface RefundAnalytics {
  workspaceId: string;
  totalRequests: number;
  totalApproved: number;
  totalRejected: number;
  totalManualReview: number;
  totalCompleted: number;
  totalFailed: number;
  totalRequestedAmount: number;
  totalRefundedAmount: number;
  approvalRate: number;
  autoApprovalRate: number;
  averageProcessingTimeMs: number;
  byReason: Record<string, number>;
  byStatus: Record<string, number>;
  byPaymentType: Record<string, number>;
  byCustomerTier: Record<string, number>;
  byDay: Record<string, number>;
  trend: Array<{ date: string; count: number; amount: number }>;
  slaBreachCount: number;
}

export interface RefundAnalyticsInput {
  workspaceId: string;
  fromDate?: string;
  toDate?: string;
  granularity?: 'day' | 'week' | 'month';
}

export interface RefundJob {
  id: string;
  refundId: string;
  workspaceId: string;
  type: 'process' | 'retry' | 'cancel';
  status: 'queued' | 'running' | 'completed' | 'failed';
  attempts: number;
  maxAttempts: number;
  lastError?: string;
  createdAt: string;
  updatedAt: string;
}

// ── In-Memory Store ──────────────────────────────────────────────────────────

const policyStore = new Map<string, RefundPolicy>();
const refundStore = new Map<string, RefundRecord>();
const jobStore = new Map<string, RefundJob>();

const refundEvents: Array<{
  workspaceId: string;
  decision: RefundDecision;
  amount: number;
  currency: string;
  reason?: string;
  customerTier?: CustomerTier;
  paymentType: PaymentMethod;
  autoProcessed: boolean;
  createdAt: string;
}> = [];

const auditLogger = new ImmutableAuditLogger();

// ── Default Policy ───────────────────────────────────────────────────────────

const defaultPolicy = (workspaceId: string): RefundPolicy => ({
  id: randomUUID(),
  workspaceId,
  name: 'default',
  fullRefundWindowDays: 30,
  autoApprovalThreshold: 100,
  alwaysRefundUnderAmount: 25,
  maxPartialRefundPct: 100,
  requireReason: true,
  firstApprovalThreshold: 500,
  secondApprovalThreshold: 5000,
  rules: [],
  isActive: true,
  createdAt: new Date().toISOString(),
  updatedAt: new Date().toISOString(),
});

// ── Policy Management ────────────────────────────────────────────────────────

export function upsertRefundPolicy(
  input: Omit<RefundPolicy, 'id' | 'createdAt' | 'updatedAt'>,
): RefundPolicy {
  const existing = Array.from(policyStore.values()).find(
    (p) => p.workspaceId === input.workspaceId && p.name === input.name,
  );

  const policy: RefundPolicy = {
    ...(existing ?? {
      id: randomUUID(),
      createdAt: new Date().toISOString(),
    }),
    ...input,
    updatedAt: new Date().toISOString(),
  };

  policyStore.set(policy.id, policy);

  auditLogger.log({
    actor: 'system',
    action: existing ? 'refund_policy.updated' : 'refund_policy.created',
    resource: `refund_policy:${policy.id}`,
    details: { workspaceId: input.workspaceId, name: input.name, rulesCount: input.rules.length },
  });

  return policy;
}

export function getRefundPolicy(workspaceId: string, name?: string): RefundPolicy {
  const policies = Array.from(policyStore.values()).filter(
    (p) => p.workspaceId === workspaceId && p.isActive,
  );
  if (name) {
    return policies.find((p) => p.name === name) ?? defaultPolicy(workspaceId);
  }
  return policies[0] ?? defaultPolicy(workspaceId);
}

export function listRefundPolicies(workspaceId: string): RefundPolicy[] {
  return Array.from(policyStore.values()).filter(
    (p) => p.workspaceId === workspaceId,
  );
}

export function deleteRefundPolicy(policyId: string): boolean {
  const policy = policyStore.get(policyId);
  if (!policy) return false;
  policy.isActive = false;
  policy.updatedAt = new Date().toISOString();
  auditLogger.log({
    actor: 'system',
    action: 'refund_policy.deleted',
    resource: `refund_policy:${policyId}`,
    details: { workspaceId: policy.workspaceId, name: policy.name },
  });
  return true;
}

// ── Rule Evaluation Engine ───────────────────────────────────────────────────

function evaluateCustomRules(
  rules: PolicyRule[],
  input: RefundEvaluationInput,
): { matched: boolean; outcome: RefundDecision; rule: PolicyRule } | null {
  const sorted = [...rules].sort((a, b) => b.priority - a.priority);

  for (const rule of sorted) {
    const fieldValue = getFieldValue(input, rule.field);
    if (fieldValue === undefined) continue;

    const matched = evaluateOperator(rule.operator, fieldValue, rule.value);
    if (matched) {
      const outcomeMap: Record<string, RefundDecision> = {
        approve: 'approved',
        reject: 'rejected',
        manual_review: 'manual_review',
      };
      return { matched: true, outcome: outcomeMap[rule.outcome] ?? 'manual_review', rule };
    }
  }
  return null;
}

function getFieldValue(input: RefundEvaluationInput, field: string): unknown {
  const fieldMap: Record<string, unknown> = {
    amount_paid: input.amountPaid,
    requested_amount: input.requestedAmount,
    days_since_payment: input.daysSincePayment,
    payment_type: input.paymentType,
    customer_tier: input.customerTier,
    customer_total_spent: input.customerTotalSpent,
    previous_refund_count: input.previousRefundCount,
    previous_refund_amount: input.previousRefundAmount,
    has_chargeback: input.hasChargeback,
    has_dispute: input.hasDispute,
    has_reason: !!input.reason?.trim(),
    reason: input.reason,
    currency: input.currency,
  };
  return fieldMap[field];
}

function evaluateOperator(operator: string, fieldValue: unknown, ruleValue: unknown): boolean {
  switch (operator) {
    case 'eq': return fieldValue === ruleValue;
    case 'neq': return fieldValue !== ruleValue;
    case 'gt': return typeof fieldValue === 'number' && typeof ruleValue === 'number' && fieldValue > ruleValue;
    case 'gte': return typeof fieldValue === 'number' && typeof ruleValue === 'number' && fieldValue >= ruleValue;
    case 'lt': return typeof fieldValue === 'number' && typeof ruleValue === 'number' && fieldValue < ruleValue;
    case 'lte': return typeof fieldValue === 'number' && typeof ruleValue === 'number' && fieldValue <= ruleValue;
    case 'in': return Array.isArray(ruleValue) && ruleValue.includes(fieldValue);
    case 'not_in': return Array.isArray(ruleValue) && !ruleValue.includes(fieldValue);
    case 'contains': return typeof fieldValue === 'string' && typeof ruleValue === 'string' && fieldValue.includes(ruleValue);
    default: return false;
  }
}

// ── Refund Evaluation Engine ─────────────────────────────────────────────────

export function evaluateRefund(input: RefundEvaluationInput): {
  decision: RefundDecision;
  reasons: string[];
  policy: RefundPolicy;
  amountApproved: number;
  requiredApprovalLevel?: 'first' | 'second' | 'third';
  matchedRule?: PolicyRule;
} {
  const policy = getRefundPolicy(input.workspaceId);
  const reasons: string[] = [];
  let decision: RefundDecision = 'manual_review';
  let requiredApprovalLevel: 'first' | 'second' | 'third' | undefined;

  if (policy.requireReason && !input.reason?.trim()) {
    throw new AppError(400, 'Refund reason is required by policy', 'ERR_REFUND_REASON_REQUIRED');
  }

  // Evaluate custom rules first (highest priority wins)
  const ruleMatch = evaluateCustomRules(policy.rules, input);
  if (ruleMatch) {
    decision = ruleMatch.outcome;
    reasons.push(`Custom policy rule matched: ${ruleMatch.rule.id} (${ruleMatch.rule.field} ${ruleMatch.rule.operator} ${ruleMatch.rule.value})`);

    if (decision === 'manual_review') {
      requiredApprovalLevel = 'first';
    }

    const refund = createRefundRecord(input, decision, reasons, policy, requiredApprovalLevel, ruleMatch.rule);
    trackRefundEvent(input, decision);

    return {
      decision,
      reasons,
      policy,
      amountApproved: decision === 'approved' ? input.requestedAmount : 0,
      requiredApprovalLevel,
      matchedRule: ruleMatch.rule,
    };
  }

  // Built-in policy rules (ordered by priority)
  if (input.hasChargeback || input.hasDispute) {
    decision = 'manual_review';
    reasons.push('Chargeback/dispute requires manual review');
    requiredApprovalLevel = 'first';
  } else if (input.requestedAmount <= policy.alwaysRefundUnderAmount) {
    decision = 'approved';
    reasons.push('Requested amount below always-refund threshold');
  } else if (
    input.daysSincePayment <= policy.fullRefundWindowDays &&
    input.requestedAmount <= input.amountPaid
  ) {
    decision = 'approved';
    reasons.push('Within full refund window');
  } else {
    const maxPartialAmount = (input.amountPaid * policy.maxPartialRefundPct) / 100;
    if (input.requestedAmount > maxPartialAmount) {
      decision = 'rejected';
      reasons.push('Requested amount exceeds partial refund policy');
    } else if (input.requestedAmount <= policy.autoApprovalThreshold) {
      decision = 'approved';
      reasons.push('Within auto-approval threshold');
    } else if (input.requestedAmount <= policy.firstApprovalThreshold) {
      decision = 'manual_review';
      requiredApprovalLevel = 'first';
      reasons.push('Requires first-level approval');
    } else if (input.requestedAmount <= policy.secondApprovalThreshold) {
      decision = 'manual_review';
      requiredApprovalLevel = 'second';
      reasons.push('Requires second-level approval');
    } else {
      decision = 'manual_review';
      requiredApprovalLevel = 'third';
      reasons.push('Requires third-level approval');
    }
  }

  const refund = createRefundRecord(input, decision, reasons, policy, requiredApprovalLevel);
  trackRefundEvent(input, decision);

  return {
    decision,
    reasons,
    policy,
    amountApproved: decision === 'approved' ? input.requestedAmount : 0,
    requiredApprovalLevel,
  };
}

function createRefundRecord(
  input: RefundEvaluationInput,
  decision: RefundDecision,
  reasons: string[],
  policy: RefundPolicy,
  requiredApprovalLevel?: 'first' | 'second' | 'third',
  matchedRule?: PolicyRule,
): RefundRecord {
  const now = new Date().toISOString();
  const autoProcessed = decision === 'approved';

  const refund: RefundRecord = {
    id: randomUUID(),
    workspaceId: input.workspaceId,
    paymentId: input.paymentId,
    paymentType: input.paymentType,
    amount: input.requestedAmount,
    currency: input.currency,
    reason: input.reason,
    status: decision === 'approved' ? 'approved' : decision === 'rejected' ? 'rejected' : 'pending',
    decision,
    decisionReasons: reasons,
    autoProcessed,
    lineItems: (input.lineItems ?? []).map((li) => ({
      label: li.label,
      amount: li.amount,
      quantity: li.quantity,
      reason: li.reason,
    })),
    approvals: requiredApprovalLevel
      ? [{ level: requiredApprovalLevel, approverId: '', status: 'pending' }]
      : [],
    history: [{
      timestamp: now,
      action: decision === 'approved' ? 'auto_approved' : decision === 'rejected' ? 'rejected' : 'pending_review',
      actor: 'system',
      fromStatus: 'pending',
      toStatus: decision === 'approved' ? 'approved' : decision === 'rejected' ? 'rejected' : 'pending',
      comment: matchedRule ? `Matched rule: ${matchedRule.id}` : undefined,
    }],
    metadata: matchedRule ? { matchedRuleId: matchedRule.id } : undefined,
    createdAt: now,
    updatedAt: now,
  };

  refundStore.set(refund.id, refund);

  auditLogger.log({
    actor: 'system',
    action: `refund.${decision}`,
    resource: `refund:${refund.id}`,
    details: {
      workspaceId: input.workspaceId,
      paymentId: input.paymentId,
      amount: input.requestedAmount,
      currency: input.currency,
      decision,
      reasons,
      autoProcessed,
      matchedRule: matchedRule?.id,
    },
  });

  return refund;
}

function trackRefundEvent(input: RefundEvaluationInput, decision: RefundDecision): void {
  refundEvents.push({
    workspaceId: input.workspaceId,
    decision,
    amount: input.requestedAmount,
    currency: input.currency,
    reason: input.reason,
    customerTier: input.customerTier,
    paymentType: input.paymentType,
    autoProcessed: decision === 'approved',
    createdAt: new Date().toISOString(),
  });
}

// ── Approval Workflow ────────────────────────────────────────────────────────

export function approveRefund(
  refundId: string,
  approverId: string,
  level: 'first' | 'second' | 'third',
  comment?: string,
): RefundRecord {
  const refund = refundStore.get(refundId);
  if (!refund) {
    throw new AppError(404, 'Refund not found', 'ERR_REFUND_NOT_FOUND');
  }

  const approval = refund.approvals.find((a) => a.level === level);
  if (!approval) {
    throw new AppError(400, `No ${level} approval required for this refund`, 'ERR_REFUND_NO_APPROVAL_NEEDED');
  }

  if (approval.status !== 'pending') {
    throw new AppError(400, `Approval at level ${level} has already been resolved`, 'ERR_REFUND_APPROVAL_RESOLVED');
  }

  const fromStatus = refund.status;

  approval.status = 'approved';
  approval.approverId = approverId;
  approval.comment = comment;
  approval.approvedAt = new Date().toISOString();

  const allApproved = refund.approvals.every((a) => a.status === 'approved');
  if (allApproved) {
    refund.status = 'approved';
    refund.decision = 'approved';
    refund.approvedBy = approverId;
  }

  refund.updatedAt = new Date().toISOString();
  refund.history.push({
    timestamp: new Date().toISOString(),
    action: 'approved',
    actor: approverId,
    fromStatus,
    toStatus: refund.status,
    comment,
  });

  refundStore.set(refundId, refund);

  auditLogger.log({
    actor: approverId,
    action: 'refund.approved',
    resource: `refund:${refundId}`,
    details: { level, comment, allApproved },
  });

  return refund;
}

export function rejectRefund(
  refundId: string,
  approverId: string,
  level: 'first' | 'second' | 'third',
  comment?: string,
): RefundRecord {
  const refund = refundStore.get(refundId);
  if (!refund) {
    throw new AppError(404, 'Refund not found', 'ERR_REFUND_NOT_FOUND');
  }

  const approval = refund.approvals.find((a) => a.level === level);
  if (!approval) {
    throw new AppError(400, `No ${level} approval required for this refund`, 'ERR_REFUND_NO_APPROVAL_NEEDED');
  }

  const fromStatus = refund.status;

  approval.status = 'rejected';
  approval.approverId = approverId;
  approval.comment = comment;
  approval.approvedAt = new Date().toISOString();

  refund.status = 'rejected';
  refund.decision = 'rejected';
  refund.updatedAt = new Date().toISOString();
  refund.history.push({
    timestamp: new Date().toISOString(),
    action: 'rejected',
    actor: approverId,
    fromStatus,
    toStatus: 'rejected',
    comment,
  });
  refundStore.set(refundId, refund);

  auditLogger.log({
    actor: approverId,
    action: 'refund.rejected',
    resource: `refund:${refundId}`,
    details: { level, comment },
  });

  return refund;
}

// ── Automated Processing ─────────────────────────────────────────────────────

export async function processRefund(
  refundId: string,
  providerFn?: (refund: RefundRecord) => Promise<{ success: boolean; txHash?: string; error?: string }>,
): Promise<RefundRecord> {
  const refund = refundStore.get(refundId);
  if (!refund) {
    throw new AppError(404, 'Refund not found', 'ERR_REFUND_NOT_FOUND');
  }

  if (refund.status !== 'approved') {
    throw new AppError(400, 'Refund must be approved before processing', 'ERR_REFUND_NOT_APPROVED');
  }

  const fromStatus = refund.status;
  refund.status = 'processing';
  refund.processedAt = new Date().toISOString();
  refund.updatedAt = new Date().toISOString();
  refund.history.push({
    timestamp: new Date().toISOString(),
    action: 'processing_started',
    actor: 'system',
    fromStatus,
    toStatus: 'processing',
  });
  refundStore.set(refundId, refund);

  try {
    if (providerFn) {
      const result = await providerFn(refund);
      if (result.success) {
        refund.status = 'completed';
        refund.completedAt = new Date().toISOString();
        if (result.txHash) {
          refund.stripeRefundId = result.txHash;
        }
      } else {
        refund.status = 'failed';
        refund.failureReason = result.error;
      }
    } else {
      refund.status = 'completed';
      refund.completedAt = new Date().toISOString();
    }

    refund.updatedAt = new Date().toISOString();
    refund.history.push({
      timestamp: new Date().toISOString(),
      action: refund.status === 'completed' ? 'completed' : 'failed',
      actor: 'system',
      fromStatus: 'processing',
      toStatus: refund.status,
      comment: refund.failureReason,
    });
    refundStore.set(refundId, refund);

    auditLogger.log({
      actor: 'system',
      action: `refund.processed.${refund.status}`,
      resource: `refund:${refundId}`,
      details: {
        status: refund.status,
        completedAt: refund.completedAt,
        failureReason: refund.failureReason,
      },
    });
  } catch (error) {
    refund.status = 'failed';
    refund.failureReason = error instanceof Error ? error.message : String(error);
    refund.updatedAt = new Date().toISOString();
    refund.history.push({
      timestamp: new Date().toISOString(),
      action: 'failed',
      actor: 'system',
      fromStatus: 'processing',
      toStatus: 'failed',
      comment: refund.failureReason,
    });
    refundStore.set(refundId, refund);

    auditLogger.log({
      actor: 'system',
      action: 'refund.failed',
      resource: `refund:${refundId}`,
      details: { error: refund.failureReason },
    });

    throw error;
  }

  return refund;
}

export async function autoProcessApprovedRefunds(
  providerFn?: (refund: RefundRecord) => Promise<{ success: boolean; txHash?: string; error?: string }>,
): Promise<{ processed: number; failed: number; errors: string[] }> {
  const approved = Array.from(refundStore.values()).filter(
    (r) => r.status === 'approved' && r.autoProcessed,
  );

  let processed = 0;
  let failed = 0;
  const errors: string[] = [];

  for (const refund of approved) {
    try {
      await processRefund(refund.id, providerFn);
      processed++;
    } catch (error) {
      failed++;
      errors.push(`Refund ${refund.id}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }

  if (processed > 0 || failed > 0) {
    auditLogger.log({
      actor: 'system',
      action: 'refund.auto_process_batch',
      resource: 'refund:batch',
      details: { processed, failed, errors },
    });
  }

  return { processed, failed, errors };
}

// ── Cancel ───────────────────────────────────────────────────────────────────

export function cancelRefund(refundId: string, actor: string, reason?: string): RefundRecord {
  const refund = refundStore.get(refundId);
  if (!refund) {
    throw new AppError(404, 'Refund not found', 'ERR_REFUND_NOT_FOUND');
  }

  if (refund.status === 'completed' || refund.status === 'cancelled') {
    throw new AppError(400, `Refund is already ${refund.status}`, 'ERR_REFUND_ALREADY_FINALIZED');
  }

  const fromStatus = refund.status;
  refund.status = 'cancelled';
  refund.cancelledAt = new Date().toISOString();
  refund.updatedAt = new Date().toISOString();
  refund.history.push({
    timestamp: new Date().toISOString(),
    action: 'cancelled',
    actor,
    fromStatus,
    toStatus: 'cancelled',
    comment: reason,
  });
  refundStore.set(refundId, refund);

  auditLogger.log({
    actor,
    action: 'refund.cancelled',
    resource: `refund:${refundId}`,
    details: { reason },
  });

  return refund;
}

// ── Query ────────────────────────────────────────────────────────────────────

export function getRefund(refundId: string): RefundRecord | undefined {
  return refundStore.get(refundId);
}

export function listRefunds(
  workspaceId: string,
  filters?: {
    status?: RefundStatus | RefundStatus[];
    paymentId?: string;
    paymentType?: PaymentMethod;
    fromDate?: string;
    toDate?: string;
    limit?: number;
    offset?: number;
    sortBy?: 'createdAt' | 'amount' | 'updatedAt';
    sortOrder?: 'asc' | 'desc';
  },
): { items: RefundRecord[]; total: number } {
  let items = Array.from(refundStore.values()).filter(
    (r) => r.workspaceId === workspaceId,
  );

  if (filters?.status) {
    const statuses = Array.isArray(filters.status) ? filters.status : [filters.status];
    items = items.filter((r) => statuses.includes(r.status));
  }
  if (filters?.paymentId) {
    items = items.filter((r) => r.paymentId === filters.paymentId);
  }
  if (filters?.paymentType) {
    items = items.filter((r) => r.paymentType === filters.paymentType);
  }
  if (filters?.fromDate) {
    items = items.filter((r) => r.createdAt >= filters.fromDate!);
  }
  if (filters?.toDate) {
    items = items.filter((r) => r.createdAt <= filters.toDate!);
  }

  const sortBy = filters?.sortBy ?? 'createdAt';
  const sortOrder = filters?.sortOrder ?? 'desc';
  items.sort((a, b) => {
    const aVal = a[sortBy] ?? '';
    const bVal = b[sortBy] ?? '';
    const cmp = aVal < bVal ? -1 : aVal > bVal ? 1 : 0;
    return sortOrder === 'desc' ? -cmp : cmp;
  });

  const total = items.length;
  const limit = filters?.limit ?? 50;
  const offset = filters?.offset ?? 0;
  items = items.slice(offset, offset + limit);

  return { items, total };
}

export function getRefundHistory(refundId: string): RefundHistoryEntry[] {
  const refund = refundStore.get(refundId);
  if (!refund) {
    throw new AppError(404, 'Refund not found', 'ERR_REFUND_NOT_FOUND');
  }
  return refund.history;
}

// ── Job Management ───────────────────────────────────────────────────────────

export function createRefundJob(
  refundId: string,
  workspaceId: string,
  type: RefundJob['type'],
): RefundJob {
  const job: RefundJob = {
    id: randomUUID(),
    refundId,
    workspaceId,
    type,
    status: 'queued',
    attempts: 0,
    maxAttempts: 3,
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  };
  jobStore.set(job.id, job);
  return job;
}

export function getRefundJob(jobId: string): RefundJob | undefined {
  return jobStore.get(jobId);
}

export function listRefundJobs(workspaceId: string, status?: RefundJob['status']): RefundJob[] {
  let jobs = Array.from(jobStore.values()).filter((j) => j.workspaceId === workspaceId);
  if (status) {
    jobs = jobs.filter((j) => j.status === status);
  }
  return jobs.sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

// ── Analytics ────────────────────────────────────────────────────────────────

export function getRefundAnalytics(input: RefundAnalyticsInput): RefundAnalytics {
  const events = refundEvents.filter((e) => e.workspaceId === input.workspaceId);
  const refunds = Array.from(refundStore.values()).filter(
    (r) => r.workspaceId === input.workspaceId,
  );

  let filteredRefunds = refunds;
  let filteredEvents = events;

  if (input.fromDate) {
    filteredRefunds = filteredRefunds.filter((r) => r.createdAt >= input.fromDate!);
    filteredEvents = filteredEvents.filter((e) => e.createdAt >= input.fromDate!);
  }
  if (input.toDate) {
    filteredRefunds = filteredRefunds.filter((r) => r.createdAt <= input.toDate!);
    filteredEvents = filteredEvents.filter((e) => e.createdAt <= input.toDate!);
  }

  const decisionCounts = filteredEvents.reduce(
    (acc, event) => {
      acc[event.decision] += 1;
      return acc;
    },
    { approved: 0, manual_review: 0, rejected: 0 } as Record<string, number>,
  );

  const byReason: Record<string, number> = {};
  const byStatus: Record<string, number> = {};
  const byPaymentType: Record<string, number> = {};
  const byCustomerTier: Record<string, number> = {};
  const byDay: Record<string, number> = {};

  let totalProcessingTimeMs = 0;
  let processingCount = 0;
  let slaBreachCount = 0;
  const slaThresholdMs = 24 * 60 * 60 * 1000; // 24 hours

  for (const refund of filteredRefunds) {
    byStatus[refund.status] = (byStatus[refund.status] || 0) + 1;
    byPaymentType[refund.paymentType] = (byPaymentType[refund.paymentType] || 0) + 1;

    if (refund.reason) {
      byReason[refund.reason] = (byReason[refund.reason] || 0) + 1;
    }

    const day = refund.createdAt.slice(0, 10);
    byDay[day] = (byDay[day] || 0) + 1;

    if (refund.processedAt && refund.completedAt) {
      const processingTime =
        new Date(refund.completedAt).getTime() - new Date(refund.processedAt).getTime();
      totalProcessingTimeMs += processingTime;
      processingCount++;
      if (processingTime > slaThresholdMs) {
        slaBreachCount++;
      }
    }
  }

  for (const event of filteredEvents) {
    if (event.customerTier) {
      byCustomerTier[event.customerTier] = (byCustomerTier[event.customerTier] || 0) + 1;
    }
  }

  const totalRequestedAmount = filteredEvents.reduce((sum, e) => sum + e.amount, 0);
  const totalRefundedAmount = filteredRefunds
    .filter((r) => r.status === 'completed')
    .reduce((sum, r) => sum + r.amount, 0);
  const autoApproved = filteredEvents.filter((e) => e.autoProcessed).length;

  // Build trend data
  const granularity = input.granularity ?? 'day';
  const trendMap = new Map<string, { count: number; amount: number }>();

  for (const event of filteredEvents) {
    let key: string;
    if (granularity === 'week') {
      const d = new Date(event.createdAt);
      const weekStart = new Date(d);
      weekStart.setDate(d.getDate() - d.getDay());
      key = weekStart.toISOString().slice(0, 10);
    } else if (granularity === 'month') {
      key = event.createdAt.slice(0, 7);
    } else {
      key = event.createdAt.slice(0, 10);
    }
    const existing = trendMap.get(key) ?? { count: 0, amount: 0 };
    existing.count++;
    existing.amount += event.amount;
    trendMap.set(key, existing);
  }

  const trend = Array.from(trendMap.entries())
    .map(([date, data]) => ({ date, ...data }))
    .sort((a, b) => a.date.localeCompare(b.date));

  const totalEvents = filteredEvents.length;

  return {
    workspaceId: input.workspaceId,
    totalRequests: totalEvents,
    totalApproved: decisionCounts.approved,
    totalRejected: decisionCounts.rejected,
    totalManualReview: decisionCounts.manual_review,
    totalCompleted: byStatus.completed ?? 0,
    totalFailed: byStatus.failed ?? 0,
    totalRequestedAmount,
    totalRefundedAmount,
    approvalRate: totalEvents > 0 ? decisionCounts.approved / totalEvents : 0,
    autoApprovalRate: totalEvents > 0 ? autoApproved / totalEvents : 0,
    averageProcessingTimeMs:
      processingCount > 0 ? totalProcessingTimeMs / processingCount : 0,
    byReason,
    byStatus,
    byPaymentType,
    byCustomerTier,
    byDay,
    trend,
    slaBreachCount,
  };
}

export function getRefundMetricsSummary(): {
  totalRefunds: number;
  totalCompleted: number;
  totalPending: number;
  totalFailed: number;
  totalAmount: number;
  completionRate: number;
} {
  const all = Array.from(refundStore.values());
  const totalRefunds = all.length;
  const totalCompleted = all.filter((r) => r.status === 'completed').length;
  const totalPending = all.filter((r) => r.status === 'pending' || r.status === 'processing' || r.status === 'approved').length;
  const totalFailed = all.filter((r) => r.status === 'failed').length;
  const totalAmount = all.filter((r) => r.status === 'completed').reduce((s, r) => s + r.amount, 0);

  return {
    totalRefunds,
    totalCompleted,
    totalPending,
    totalFailed,
    totalAmount,
    completionRate: totalRefunds > 0 ? totalCompleted / totalRefunds : 0,
  };
}

// ── Stripe Integration ───────────────────────────────────────────────────────

export async function processRefundWithProvider(
  refundId: string,
  providerFn: (refund: RefundRecord) => Promise<{ success: boolean; txHash?: string; error?: string }>,
): Promise<RefundRecord> {
  return processRefund(refundId, providerFn);
}

// ── Retry Failed Refunds ─────────────────────────────────────────────────────

export async function retryFailedRefund(
  refundId: string,
  providerFn?: (refund: RefundRecord) => Promise<{ success: boolean; txHash?: string; error?: string }>,
): Promise<RefundRecord> {
  const refund = refundStore.get(refundId);
  if (!refund) {
    throw new AppError(404, 'Refund not found', 'ERR_REFUND_NOT_FOUND');
  }

  if (refund.status !== 'failed') {
    throw new AppError(400, 'Only failed refunds can be retried', 'ERR_REFUND_NOT_FAILED');
  }

  refund.status = 'approved';
  refund.failureReason = undefined;
  refund.updatedAt = new Date().toISOString();
  refund.history.push({
    timestamp: new Date().toISOString(),
    action: 'retry',
    actor: 'system',
    fromStatus: 'failed',
    toStatus: 'approved',
  });
  refundStore.set(refundId, refund);

  return processRefund(refundId, providerFn);
}

// ── Test Helpers ─────────────────────────────────────────────────────────────

export function resetForTests(): void {
  policyStore.clear();
  refundStore.clear();
  jobStore.clear();
  refundEvents.length = 0;
}
